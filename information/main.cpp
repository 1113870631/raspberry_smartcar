...
#include <mavlink.h>
#include "./HC-SR04-Raspberry-Pi-C-/distance.h"
...
// Consumed within ArduPilot by the proximity class
  uint8_t covariance = 0; /*< Measurement covariance in centimeters, 0 for unknown / invalid readings*/
	...
	for (;;) 
    {
		...
		/*send distance*/
		//current_distance++;
		current_distance=Get_distance( sonar);
		 mavlink_msg_distance_sensor_pack(1,200,&msg,time_boot_ms,min_distance,max_distance,current_distance,type,id,orientation,covariance,0,0,0,0);
		 len = mavlink_msg_to_send_buffer(buf, &msg);	
		 bytes_sent = sendto(sock, buf, len, 0, (struct sockaddr*)&gcAddr, sizeof (struct sockaddr_in));
